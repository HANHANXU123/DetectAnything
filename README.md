...
深色无边框主界面：**左侧**为推理结果画布 + 实时日志面板，**右侧**自上而下为「任务类型 / 数据源 / 运行控制」三区，**底部状态栏**实时显示推理耗时(ms)、真实帧率(FPS) 与系统时钟。以下为各已实现任务的实测运行截图：
...
**无监督异常检测 Anomaly Detection**

<img src="docs/anomaly.png" alt="无监督异常检测" width="500">
