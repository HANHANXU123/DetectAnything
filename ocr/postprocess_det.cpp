...
static float box_score(const Mat& pred, const vector<vector<float>>& box_array){
    ...
    int xmin = clamp_val(int(std::floor(*(std::min_element(box_x, box_x + 4)))), 0, width - 1);
    int xmax = clamp_val(int(std::ceil(*(std::max_element(box_x, box_x + 4)))), 0, width - 1);
    int ymin = clamp_val(int(std::floor(*(std::min_element(box_y, box_y + 4)))), 0, height - 1);
    int ymax = clamp_val(int(std::ceil(*(std::max_element(box_y, box_y + 4)))), 0, height - 1);
...
static cv::RotatedRect box_unclip(const vector<vector<float>>& box, float unclip_ratio){
    ...
    for(int i = 0; i < soln.size(); ++i){
        for(int j = 0; j < soln[i].size(); ++j){
            points.emplace_back(float(soln[i][j].X), float(soln[i][j].Y));
        }
    }
    cv::RotatedRect res;
    if(points.size() <= 0){
...
static void boxes_from_bitmap(
    ...
    for(auto& contour : contours){
        ...
        for(int i = 0; i < 4; ++i){
            int x = int(clamp_val(std::roundf(cliparray[i][0] * x_scale), 0.0f, float(dest_width)));
            int y = int(clamp_val(std::roundf(cliparray[i][1] * y_scale), 0.0f, float(dest_height)));
